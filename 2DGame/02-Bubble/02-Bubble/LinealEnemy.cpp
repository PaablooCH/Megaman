...
void LinealEnemy::update(int deltaTime)
{
	...
	case MOVING_LEFT: {
		...
		if (map->checkPlayerLeft(posEnemy, glm::ivec2(20, 32)))
		{
		...
		/*if (map->checkEnemyBulletDamageLeft(posEnemy, glm::ivec2(32, 32)))
		{
			isAlive = false;
			map->clearPosition(ID);
		}*/
	...
	case MOVING_RIGHT: {
		...
		if (map->checkEnemyBulletDamage(posEnemy, glm::ivec2(32, 32)))
		{
			isAlive = false;
			map->clearPosition(ID);
		}
		/*if (map->checkEnemyBulletDamageRight(posEnemy, glm::ivec2(16, 32)))
		{
			isAlive = false;
			map->clearPosition(ID);
		}*/
